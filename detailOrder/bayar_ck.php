...
<?php if (isset($_POST['bayar_ck'])) { ?>
   <?php if (transaksi_ck($_POST) > 0) : ?>
      <div class="alert">
         <div class="box">
         <img src="<?=url('Assets_image/berhasil.png')?>" height="68" alt="alert sukses">
            <p>Pembayaran Berhasil!</p>
            <button onclick="window.location='http: //localhost/CleanCo_Laundry/riwayat.php'" class="btn-alert btn-success">Ok</button>
         </div>
      </div>
      <?php else :?>
      <div class="alert">
         ...
         <img src="<?=url('_assets/img/gagal.png')?>" height="68" alt="alert gagal">
            ...
            <a href="<?=url("detail_order/detail_ck/bayar_ck.php?or_ck_number=$nomor_or")?>" class="btn-alert btn-fail">Ok</a>
         </div>
      </div>
   <?php endif ?>
<?php } ?>
...
<head>
   ...
   <link rel="shortcut icon" href="<?=url('Assets/image/logo/favicon.svg')?>" type="image/x-icon">
</head>
<body>
